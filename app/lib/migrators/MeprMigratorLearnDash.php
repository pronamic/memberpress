...
use memberpress\courses\models\Answer;
use memberpress\courses\models\Attempt;
use memberpress\courses\models\Course;
use memberpress\courses\models\Lesson;
use memberpress\courses\models\Question;
...
class MeprMigratorLearnDash extends MeprMigrator {
  ...
  public function migrate(array $data) {
    ...
    wp_send_json_error(__('Bad request', 'easy-affiliate', 'memberpress'));
  }
  ...
  protected function check_prerequisites(array $data) {
    ...
    foreach($tables as $table) {
      if(!$mepr_db->table_exists($table)) {
        wp_send_json_error(sprintf(__('Migration not started: table `%s` not found, try updating LearnDash to the latest version first.', 'memberpress'), $table));
      }
    }

    if(defined('memberpress\\courses\\VERSION')) {
      if(version_compare(memberpress\courses\VERSION, '1.2.21', '<=')) {
        wp_send_json_error(__('Please update the MemberPress Courses plugin to the latest version and try again.', 'memberpress'));
      }
    }
    else {
      if(file_exists(WP_PLUGIN_DIR . '/memberpress-courses/main.php')) {
        if(empty($data['retry'])) {
          ...
          if(
            ...
            version_compare(memberpress\courses\VERSION, '1.2.21', '>') &&
            ...
            $courses_slug = LearnDash_Settings_Section::get_section_setting('LearnDash_Settings_Section_Permalinks', 'courses');
            $lessons_slug = LearnDash_Settings_Section::get_section_setting('LearnDash_Settings_Section_Permalinks', 'lessons');
            $quizzes_slug = LearnDash_Settings_Section::get_section_setting('LearnDash_Settings_Section_Permalinks', 'quizzes');
            ...
            if($quizzes_slug == Lessons::get_quizzes_permalink_base()) {
          ...
          $this->send_success_response($data, ['step' => 'start', 'retry' => 'true']);
        }

        wp_send_json_error(__('Please activate the MemberPress Courses plugin and try again.', 'memberpress'));
      }
      else {
        wp_send_json_error(__('Please install and activate the MemberPress Courses plugin and try again.', 'memberpress'));
      }
    }
  ...
  protected function migrate_taxonomies(array $data) {
    ...
    $this->send_success_response($data, ['step' => 'courses', 'logs' => $this->logs]);
  }
  ...
  protected function migrate_courses(array $data) {
    ...
    if(empty($data['options']['userProgress'])) {
      ...
      $this->send_success_response($data, ['status' => 'complete', 'logs' => $this->logs]);
    }
    else {
      $this->send_success_response($data, ['step' => 'progress', 'logs' => $this->logs]);
    }
  }
  ...
  protected function migrate_course_taxonomies($course_id, $ld_course_id) {
    ...
    if(count($taxonomies)) {
      $categories = array_intersect(array_keys($taxonomies), ['ld_course_category', 'category']);
      ...
      $tags = array_intersect(array_keys($taxonomies), ['ld_course_tag', 'post_tag']);
      ...
      if(count($section_data['children'])) {
        foreach($section_data['children'] as $lesson_order => $ld_lesson_id) {
          ...
          if($ld_lesson instanceof WP_Post) {
            if(!in_array($ld_lesson->post_type, ['sfwd-lessons', 'sfwd-topic', 'sfwd-quiz'], true)) {
              continue;
            }
            ...
            try {
              ...
              $this->logs[] = $this->model_migration_failed_log(
                $ld_lesson->post_type == 'sfwd-quiz' ? Quiz::class : Lesson::class,
                !empty($lesson_data['post_title']) ? $lesson_data['post_title'] : __('Untitled', 'memberpress'),
                $ld_lesson->ID,
                $e->getMessage()
              );
            }
    ...
    if(is_numeric($quiz_pro_id)) {
      ...
      if(!empty($ld_questions)) {
        ...
        foreach($ld_questions as $i => $ld_question) {
          ...
          switch($ld_question->answer_type) {
            case 'single':
              ...
              foreach($answer_data as $index => $option) {
                $question_data['options'][] = isset($option['_answer']) && is_string($option['_answer']) ? $option['_answer'] : '';
            ...
            case 'matrix_sort_answer':
              ...
              foreach($answer_data as $option) {
                $question_data['options'][] = isset($option['_answer']) && is_string($option['_answer']) ? $option['_answer'] : '';
                $question_data['answer'][] = isset($option['_sortString']) && is_string($option['_sortString']) ? $option['_sortString'] : '';
              }
              break;
            case 'cloze_answer':
              $question_data['answer'] = $this->convert_fill_blank_answer(
                isset($answer_data[0]['_answer']) && is_string($answer_data[0]['_answer']) ? $answer_data[0]['_answer'] : ''
              );
              break;
            case 'assessment_answer':
              list($question_data['options'], $question_data['settings']) = $this->convert_likert_scale_answer(
                isset($answer_data[0]['_answer']) && is_string($answer_data[0]['_answer']) ? $answer_data[0]['_answer'] : ''
              );
              break;
            default:
          ...
          $existing_id = !empty($migrated_questions[$ld_question->id]) ? $migrated_questions[$ld_question->id] : 0;
        ...
        if($quiz->post_content !== '' && count($questions)) {
          $content[] = ''; // adds two newlines below, after existing post content
        }

        foreach($questions as $question) {
          $content[] = "<!-- wp:memberpress-courses/$question->type-question {\"questionId\":$question->id} /-->";
        }
   ...
   *
   * @param  int    $ld_course_id
   * @param  int    $ld_lesson_id
   * @param  string $post_type
   * @return int
   */
  protected function get_existing_lesson_id($ld_course_id, $ld_lesson_id, $post_type) {
    global $wpdb;
   ...
   * @return array
   */
  protected function get_course_sections($ld_course_id) {
   ...
   * @return array
   */
  protected function populate_course_sections($ld_course_id, $sections) {
    $ld_course_steps = get_post_meta($ld_course_id, 'ld_course_steps', true);
    $steps = isset($ld_course_steps['steps']['h']) && is_array($ld_course_steps['steps']['h']) ? $ld_course_steps['steps']['h'] : [];
   ...
   * @return array
   */
  protected function filter_existing_sections($course_sections, $uuids) {
    $existing_sections = [];
   ...
   *
   * If LD is not active, we can grab the values from the __PHP_Incomplete_Class.
   *
   * @param string $answer_data Serialized answer data.
   * @return array
   */
  protected function parse_question_answer_data($answer_data) {
   ...
   * @return string
   */
  protected function convert_fill_blank_answer($answer) {
    if(!is_string($answer)) {
      return '';
    }
  ...
  protected function migrate_progress(array $data) {
    ...
    $fetch_batch = function ($limit, $offset) {
      ...
      $user_activity = $wpdb->get_results($wpdb->prepare($query, Lesson::$cpt, Course::$cpt, Lesson::$cpt, $limit, $offset));
    ...
    $this->send_success_response($data, ['step' => 'attempts', 'logs' => $this->logs]);
  }
  ...
  protected function migrate_attempts(array $data) {
    ...
    $fetch_batch = function ($limit, $offset) {
      ...
      $user_activity = $wpdb->get_results($wpdb->prepare($query, Quiz::$cpt, Course::$cpt, Lesson::$cpt, $limit, $offset));
    ...
    while($processor->next_batch()) {
      ...
      foreach($user_activity as $activity) {
        ...
        $attempt = Attempt::get_one(['quiz_id' => $quiz_id, 'user_id' => $user_id]);
        ...
        foreach($answers as $answer) {
          ...
          $question_id = !empty($migrated_questions[$ld_question_id]) ? (int) $migrated_questions[$ld_question_id] : 0;
  ...
  public static function is_migration_possible(): bool {
    ...
    $ld_course_exists = (bool) $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_type = 'sfwd-courses' LIMIT 1");
