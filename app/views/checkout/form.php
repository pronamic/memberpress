  ...
  <form name="mepr_signup_form" id="mepr_signup_form" class="mepr-signup-form mepr-form" method="post" action="<?php echo esc_url(esc_url_raw(wp_unslash($_SERVER['REQUEST_URI'] ?? '')) . '#mepr_jump'); ?>" enctype="multipart/form-data" novalidate>
    ...
    <?php if ($payment_required || !empty($product->plan_code)) : ?>
        ...
        <div class="mp-form-row mepr_coupon mepr_coupon_<?php echo esc_attr($product->ID); ?> mepr-hidden">
          <div class="mp-form-label">
            <label for="mepr_coupon_code-<?php echo esc_attr($product->ID); ?>"><?php echo esc_html_x('Coupon Code:', 'ui', 'memberpress'); ?></label>
            <span class="mepr-coupon-loader mepr-hidden">
              <img src="<?php echo esc_url(includes_url('js/thickbox/loadingAnimation.gif')); ?>" title="<?php echo esc_attr_x('Loading icon', 'ui', 'memberpress'); ?>" alt="<?php echo esc_attr_x('Loading icon', 'ui', 'memberpress'); ?>" width="100" height="10" />
            </span>
      ...
      <div class="mepr-payment-methods-wrapper">
        <?php echo MeprOptionsHelper::payment_methods_dropdown('mepr_payment_method', $product->payment_methods(), $product); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
      </div>
    <?php else : ?>
